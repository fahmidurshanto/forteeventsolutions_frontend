...
const GALLERY_IMAGES = [
  "/Gazebo%20Tent/gazebo_tent_1.webp",
  "/Gazebo%20Tent/gazebo_tent_2.webp",
  "/Gazebo%20Tent/gazebo_tent_3.webp",
  "/Gazebo%20Tent/gazebo_tent_4.webp",
  "/Gazebo%20Tent/gazebo_tent_5.webp",
  "/Gazebo%20Tent/gazebo_tent_6.webp",
];

export default function GazeboTentPage() {
  return (
    <main className="min-h-screen bg-[#0F172A]">
      <RentalHero title="Gazebo Tent" image="/Gazebo%20Tent/gazebo_tent_1.webp" />
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex items-center gap-2 text-sm font-bold italic">
          ...
          <span className="text-[#B48E4B]">Gazebo Tent</span>
        </div>
      </div>
      ...
      <section className="py-16 bg-[#131B2E] border-b border-white/5 px-4">
        <div className="max-w-3xl mx-auto text-center space-y-6">
          <p className="text-gray-300 text-lg leading-relaxed italic font-medium opacity-90">
            The Gazebo Tent is an elegant, portable shelter perfect for garden parties, roadshows,
            outdoor fairs, and private functions. Its pop-up design allows for rapid deployment while
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {GALLERY_IMAGES.map((img, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} whileHover={{ y: -8 }} className="relative aspect-[4/3] rounded-[24px] overflow-hidden shadow-2xl border border-white/10 group">
                <Image src={img} alt={`Gazebo Tent ${i + 1}`} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover transition-transform duration-700 group-hover:scale-110" />
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <div className="space-y-8">
            <h2 className="text-4xl md:text-5xl font-black text-white uppercase italic tracking-tighter leading-tight">
              Why Choose Our <span className="text-[#B48E4B]">Gazebo Tent</span>?
            </h2>
            <ul className="space-y-5">
              {["Portable pop-up design for fast and hassle-free setup", "Elegant appearance suitable for upscale garden parties", "Lightweight yet sturdy frame with water-resistant canopy", "Perfect for roadshows, outdoor fairs, and private functions", "Available with optional side walls for added protection", "Custom branding and printing options for corporate events"].map((f, i) => (
          ...
          <div className="relative aspect-[4/3] rounded-[3rem] overflow-hidden shadow-2xl group border-8 border-white/5">
            <Image src="/Gazebo%20Tent/gazebo_tent_2.webp" alt="Gazebo Tent" fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover transition-transform duration-1000 group-hover:scale-110" />
